@extends('pages.master')
@section('body')
	<div class="col-md-8 paddingtop">
		<h2 title="">Hệ thống thiết bị</h2>
		<hr>
		<table border="1" cellpadding="0" cellspacing="0">
			<tbody>
				<tr>
					<td style="width:207px;height:51px;">
						<p align="center"><strong>T&Ecirc;N THIẾT BỊ</strong></p>
					</td>
					<td style="width:90px;height:51px;">
						<p align="center"><strong>SỐ LƯỢNG</strong></p>
					</td>
					<td style="width:259px;height:51px;">
						<p align="center"><strong>NĂNG LỰC</strong></p>
					</td>
				</tr>
				<tr>
					<td style="width:207px;height:30px;">
						<p>&nbsp;Cẩu Gottwald</p>
					</td>
					<td style="width:90px;height:30px;">
						<p style="text-align: center;">01</p>
					</td>
					<td style="width:259px;height:30px;">
						<p>&nbsp;Sức n&acirc;ng 100T, tầm với 36m</p>
					</td>
				</tr>
				<tr>
					<td style="width:207px;">
						<p>&nbsp;Cẩu ch&acirc;n đế cố định</p>
					</td>
					<td style="width:90px;">
						<p style="text-align: center;">02</p>
					</td>
					<td style="width:259px;">
						<p>&nbsp;Sức n&acirc;ng 40T, tầm với 32m</p>
					</td>
				</tr>
				<tr>
					<td style="width:207px;">
						<p>&nbsp;Cẩu ch&acirc;n đế di động</p>
					</td>
					<td style="width:90px;">
						<p style="text-align: center;">04</p>
					</td>
					<td style="width:259px;">
						<p>&nbsp;Sức n&acirc;ng 40T, tầm với 36m</p>
					</td>
				</tr>
				<tr>
					<td style="width:207px;">
						<p>&nbsp;Cổng trục b&atilde;i</p>
					</td>
					<td style="width:90px;">
						<p align="center">03</p>
					</td>
					<td style="width:259px;">
						<p>&nbsp;Sức n&acirc;ng 25T</p>
					</td>
				</tr>
				<tr>
					<td style="width:207px;">
						<p>&nbsp;Cổng trục n&acirc;ng Container</p>
					</td>
					<td style="width:90px;">
						<p align="center">02</p>
					</td>
					<td style="width:259px;">
						<p>&nbsp;Sức n&acirc;ng 45 Tấn</p>
					</td>
				</tr>
				<tr>
					<td style="width:207px;">
						<p>&nbsp;Cẩu b&aacute;nh x&iacute;ch</p>
					</td>
					<td style="width:90px;">
						<p align="center">01</p>
					</td>
					<td style="width:259px;">
						<p>&nbsp;Sức n&acirc;ng 150 Tấn</p>
					</td>
				</tr>
				<tr>
					<td style="width:207px;">
						<p>&nbsp;Cẩu b&aacute;nh x&iacute;ch</p>
					</td>
					<td style="width:90px;">
						<p align="center">01</p>
					</td>
					<td style="width:259px;">
						<p>&nbsp;Sức n&acirc;ng 60 Tấn</p>
					</td>
				</tr>
				<tr>
					<td style="width:207px;">
						<p>&nbsp;T&agrave;u lai</p>
					</td>
					<td style="width:90px;">
						<p align="center">02</p>
					</td>
					<td style="width:259px;">
						<p>&nbsp;3.500Hp, 2.000Hp</p>
					</td>
				</tr>
				<tr>
					<td style="width:207px;height:65px;">
						<p>&nbsp;Xe n&acirc;ng h&agrave;ng c&aacute;c loại</p>
					</td>
					<td style="width:90px;height:65px;">
						<p style="text-align: center;">04</p>

						<p style="text-align: center;">01</p>

						<p style="text-align: center;">05</p>
					</td>
					<td style="width:259px;height:65px;">
						<p>&nbsp;Sức n&acirc;ng 25T</p>

						<p>&nbsp;Sức n&acirc;ng 8T</p>

						<p>&nbsp;Sức năng 3.5T</p>
					</td>
				</tr>
				<tr>
					<td style="width:207px;">
						<p>&nbsp;Xe n&acirc;ng Container</p>
					</td>
					<td style="width:90px;">
						<p style="text-align: center;">02</p>
					</td>
					<td style="width:259px;">
						<p>&nbsp;Sức n&acirc;ng 45 tấn, tầm với 15m</p>
					</td>
				</tr>
				<tr>
					<td style="width:207px;height:61px;">
						<p>&nbsp;Xe cuốc gầu</p>
					</td>
					<td style="width:90px;height:61px;">
						<p style="text-align: center;">06</p>

						<p style="text-align: center;">04</p>
					</td>
					<td style="width:259px;height:61px;">
						<p>&nbsp;Dung t&iacute;ch g&agrave;u 1.6M<sup>3</sup></p>

						<p>&nbsp;Dung t&iacute;ch g&agrave;u&nbsp; &nbsp;0.93M<sup>3</sup>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</p>
					</td>
				</tr>
				<tr>
					<td style="width:207px;height:59px;">
						<p>&nbsp;Xe ủi</p>
					</td>
					<td style="width:90px;height:59px;">
						<p style="text-align: center;">​03</p>

						<p style="text-align: center;">03</p>
					</td>
					<td style="width:259px;height:59px;">
						<p>&nbsp;D41</p>

						<p>&nbsp;D61</p>
					</td>
				</tr>
				<tr>
					<td style="width:207px;height:59px;">
						<p>&nbsp;X&uacute;c lật</p>
					</td>
					<td style="width:90px;height:59px;">
						<p style="text-align: center;">06</p>

						<p style="text-align: center;">04</p>

						<p style="text-align: center;">01</p>
					</td>
					<td style="width:259px;height:59px;">
						<p>&nbsp;Dung t&iacute;ch g&agrave;u 5M<sup>3</sup><sup>; </sup></p>

						<p>&nbsp;Dung t&iacute;ch g&agrave;u 3,5M<sup>3</sup><sup>;</sup></p>

						<p>&nbsp;Dung t&iacute;ch g&agrave;u 1,5M<sup>3</sup><sup>; </sup></p>
					</td>
				</tr>
				<tr>
					<td style="width:207px;height:59px;">
						<p>&nbsp;Xe qu&eacute;t r&aacute;c</p>
					</td>
					<td style="width:90px;height:59px;">
						<p style="text-align: center;">​01</p>

						<p style="text-align: center;">02</p>
					</td>
					<td style="width:259px;height:59px;">
						<p>&nbsp; 0.62 M<sup>3</sup></p>

						<p>&nbsp;10 M<sup>3</sup></p>
					</td>
				</tr>
				<tr>
					<td style="width:207px;height:59px;">
						<p>&nbsp;Xe dọn vệ sinh hầm t&agrave;u</p>
					</td>
					<td style="width:90px;height:59px;">
						<p style="text-align: center;">04</p>
					</td>
					<td style="width:259px;height:59px;">
						<p>&nbsp;0.5 M<sup>3</sup></p>
					</td>
				</tr>
				<tr>
					<td style="width:207px;height:59px;">
						<p>&nbsp;Xe rửa bến b&atilde;i</p>
					</td>
					<td style="width:90px;height:59px;">
						<p style="text-align: center;">01</p>
					</td>
					<td style="width:259px;height:59px;">
						<p>10 M<sup>3</sup></p>
					</td>
				</tr>
				<tr>
					<td style="width:207px;">
						<p>&nbsp;Gầu ngoạm thủy lực</p>
					</td>
					<td style="width:90px;">
						<p style="text-align: center";>​05</p>

						<p style="text-align: center";>06</p>
					</td>
					<td style="width:259px;">
						<p>&nbsp;20 M<sup>3</sup></p>

						<p>&nbsp;6 M<sup>3</sup></p>
					</td>
				</tr>
				<tr>
					<td style="width:207px;">
						<p>&nbsp;Gầu ngoạm hoa thị</p>
					</td>
					<td style="width:90px;">
						<p style="text-align: center;">​09</p>

						<p style="text-align: center;">10</p>
					</td>
					<td style="width:259px;">
						<p>&nbsp;6 M<sup>3</sup></p>

						<p>&nbsp;0.45 M<sup>3</sup></p>
					</td>
				</tr>
				<tr>
					<td style="width:207px;">
						<p>&nbsp;B&agrave;n c&acirc;n</p>
					</td>
					<td style="width:90px;">
						<p align="center">02</p>
					</td>
					<td style="width:259px;">
						<p>120T</p>
					</td>
				</tr>
				<tr>
					<td style="width:207px;">
						<p>&nbsp;Phễu</p>
					</td>
					<td style="width:90px;">
						<p style="text-align: center;">03</p>
					</td>
					<td style="width:259px;">
						<p>L&agrave;m h&agrave;ng rời như thạch cao, than, c&aacute;m v.v.</p>
					</td>
				</tr>
				<tr>
					<td style="width:207px;">
						<p>&nbsp;Khung chụp Container</p>
					</td>
					<td style="width:90px;">
						<p style="text-align: center;">04</p>

						<p style="text-align: center;">04</p>
					</td>
					<td style="width:259px;">
						<p>&nbsp;20F</p>

						<p>&nbsp;40F</p>
					</td>
				</tr>
				<tr>
					<td style="width:207px;">
						<p>&nbsp;Băng tải gỗ dăm</p>
					</td>
					<td style="width:90px;">
						<p style="text-align: center;">1</p>
					</td>
					<td style="width:259px;">
						<p>1200 Tấn/h</p>
					</td>
				</tr>
				<tr>
					<td style="width:207px;">
						<p>&nbsp;Băng tải chuyển đ&aacute;, clinker</p>
					</td>
					<td style="width:90px;">
						<p style="text-align: center;">1</p>
					</td>
					<td style="width:259px;">
						<p>400 Tấn/h</p>
					</td>
				</tr>
			</tbody>
		</table>
		<p></p>
		<p><img src="img/hethong1.jpg" alt="" width="500px"></p>
		<p><img src="img/hethong2.jpg" alt="" width="500px"></p>
		<p><img src="img/hethong3.jpg" alt="" width="500px"></p>
	</div>
@endsection